...
function Table({songs}) {
    ...
    useEffect(()=>{
        let trackIDs = songs.filter(song=>song.id!==null && followingStatus[song.id]===undefined).map(tr=>tr.id);
                ...
                <tbody>
                    {
                        songs.map(song=><tr key={song.id}>
                            ...
                            <td>
                                {
                                    followingStatus[song.id] ? 
                                    <svg role="img" height="16" width="16" viewBox="0 0 16 16" onClick={()=>{
                                        dispatch(unFollowTrack(accessToken,song.id));
                                    ...
                                    <svg style={{visibility:'hidden'}} role="img" height="16" width="16" viewBox="0 0 16 16" onClick={()=>{
                                        dispatch(followTrack(accessToken,song.id));
