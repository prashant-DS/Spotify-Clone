...
function PlaylistPage() {
    ...
    const savedPlaylist = useSelector(state=>state.userCollection.savedData.playlists);
    ...
    const followingStatus = useSelector(state=>state.userCollection.following.isFollowing[playlistID]);
    const userId = useSelector(state=>state.authentication.userProfile.id);
    
    useEffect(()=>{
        if(followingStatus===undefined)
            dispatch(fetchUserIsFollowingPlaylist(accessToken,userId,playlistID));
                ...
                playlistDetails===undefined ? <img className={baseStyle.loading} src={loadingImg} alt='loading'/> :<>
                    <div className={Style.introdiv} ref={introdivRef}>
                        ...
                        <div>
                            ...
                            <p className={Style.extra}>{playlistDetails.description}</p>
                            <p className={Style.extra}>
                                <span className={Style.boldextra}>{playlistDetails.owner.display_name}</span>
                                <span className={Style.dot}>•</span>
                                <span>{playlistDetails.followers.total} likes</span>
                                <span className={Style.dot}>•</span>
                    ...
                    <Table songs={playlistDetails.tracks.items}/>
                </>
            }
